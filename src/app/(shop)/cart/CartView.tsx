...
export default function CartView({
  /**
   * From the merchant's checkout settings. The same flag gates the checkout
   * page's coupon box, so the two surfaces cannot disagree about whether
   * coupons are on offer.
   */
  showCouponBox,
}: {
  ...
  if (isLoading) {
    return (
      <div className="container-px flex site-container justify-center py-20 text-gray-400">
  ...
  if (isError) {
    return (
      <div className="container-px site-container py-20 text-center">
  ...
  return (
    <div className="container-px site-container py-10">
      ...
      {cart.lines.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
          ...
          <div className="h-fit rounded-xl bg-gray-50 p-6">
            ...
            {/* Only the entry box is hidden. A discount already applied stays
                honoured and stays visible in the line above — switching the box
                off must not silently revoke a coupon a shopper already used. */}
            {showCouponBox && (
              <CouponForm
